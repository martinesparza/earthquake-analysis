...
parser = argparse.ArgumentParser(
    description="Pre/post perturbation kinematics encoding — predict neural PCs from kinematics"
)
parser.add_argument(
    "--sessions",
    nargs="+",
...
parser.add_argument(
    ...
    default=ALL_AREAS,
    metavar="AREA",
    help="Brain area(s) to encode into (default: all areas)",
)
parser.add_argument(
    "--keypoints",
    nargs="+",
    ...
    metavar="KP",
    help="Keypoint(s) to use as input features (default: all keypoints)",
)
parser.add_argument(
    "--pre-start",
    type=float,
...
# Target: canonical PC counts per area
...
def score_window(kin_arr, neural_arr, t_start, t_end, model, cv):
    """
    Cross-validated R² for encoding: predict neural_arr from kin_arr.
    X = kinematics (trials, time, n_kin_features) → flattened over time
    y = neural PCs (trials, time, n_pcs) → flattened over time
    """
    X = kin_arr[:, t_start:t_end, :].reshape(-1, kin_arr.shape[-1])
    y = neural_arr[:, t_start:t_end, :].reshape(-1, neural_arr.shape[-1])
...
for session, val in all_session_processed.items():
    ...
    for area in AREAS:
        ...
        for keypoint in tqdm(KEYPOINTS, desc=f"{area} | keypoints"):
            ...
            kin_ = np.stack(perturb_td_shuff[keypoint].values)  # (trials, T, 3)

            results_kp = {}
            for win_name, (t0, t1) in WINDOWS_BINS.items():
                scores = score_window(kin_, neural_data, t0, t1, model, cv)
                results_kp[win_name] = scores  # (n_folds,)

            results_area[keypoint] = results_kp

        out_path = f"{RESULTS_DIR}kin_enc_pre_post_{area}_{session}_npcs{n_pcs}.pkl"
