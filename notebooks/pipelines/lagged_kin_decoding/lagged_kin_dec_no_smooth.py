...
parser.add_argument(
    "--results-dir",
    default="/data/equake_results/lagged_kin_dec_no_smoothing/",
    metavar="DIR",
...
for session, val in all_session_processed.items():
    ...
    for pc_multiplier in [4]:
        for area in AREAS[:2]:
            ...
            n_pcs = min(
                pc_dict[area] * pc_multiplier, perturb_td_shuff[pca_col].iloc[0].shape[-1]
            )

            neural_data = np.stack(perturb_td_shuff[pca_col].values)[:, :, :n_pcs]
            print(f"\n{session} | {area} | {n_pcs} PCs (x{pc_multiplier})")
            ...
            for keypoint in tqdm(KEYPOINTS, desc=f"{area} | keypoints"):
                ...
                z = (power_ - power_.mean()) / (power_.std() + 1e-8)
                valid_trials = (np.abs(z) <= 2).all(axis=(1, 2))
                n_dropped = (~valid_trials).sum()
                print(
                    f"  {keypoint}: dropped {n_dropped}/{len(valid_trials)} trials with outliers"
                )
                power_ = power_[valid_trials]
                neural_data_kp = neural_data[valid_trials]

                # alpha = opt_ridge_alpha(
                #     neural_data_kp.reshape(-1, neural_data_kp.shape[-1]),
                ...
                for t_s, t in zip(TIME_BINS_S, _time_bins):
                    ...
                    per_lag = Parallel(n_jobs=-1, prefer="processes")(
                        delayed(score_one_lag)(
                            neural_data_kp, t, lag, y, model, cv, _window_bins
                        )
                        for lag in _lags_bins
                    )
                    results_kp[t_s + WINDOW_S] = np.stack(
                        per_lag, axis=0
                    )  # (n_lags, n_folds)

                results_session[keypoint] = results_kp

            out_path = f"{RESULTS_DIR}lagged_dec_{area}_{session}_npcs{n_pcs}.pkl"
