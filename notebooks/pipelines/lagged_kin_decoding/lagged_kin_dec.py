...
# ---------------------------------------------------------------------------
# All available sessions and keypoints (used as defaults)
# ---------------------------------------------------------------------------
ALL_SESSIONS = [
...
# CLI arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Lagged kinematics decoding pipeline")
parser.add_argument(
    "--sessions",
    nargs="+",
...
parser.add_argument(
    ...
    default=["left_elbow"],
    metavar="KP",
    help="Keypoint(s) to decode (default: left_elbow)",
)
parser.add_argument(
    "--window",
    type=float,
...
parser.add_argument(
    ...
    default=-0.15,
    metavar="S",
    help="Minimum lag in seconds (default: -0.20)",
)
parser.add_argument(
    "--lag-max",
    type=float,
    default=0.16,
    metavar="S",
    help="Maximum lag in seconds (default: 0.21)",
)
parser.add_argument(
    "--results-dir",
    default="/data/equake_results/lagged_kin_dec_20_pcs/",
    metavar="DIR",
...
for session, val in all_session_processed.items():
    ...
    for area in AREAS:
        ...
        # Use all available PCs for this area
        ...
        for keypoint in tqdm(KEYPOINTS, desc=f"{area} | keypoints"):
            ...
            # Fit alpha on all timepoints (original strategy)
