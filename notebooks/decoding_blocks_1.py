...
for session_idx in range(len(prep_dfs)):
    ...
    field = "MOp_rates"
    # --- prerequisites: prep_df already built, good units filtered, etc. ---
    # Make sure bhv exists for ALL trials (including free0)
    prep_df, _ = dt.add_bhv(prep_df, bhv_fields=["all_keypoints"])

    # ----- make windows for free0 -----
    free0_len = get_n_time(prep_df, "free0", field=field)  # in bins
    windows0 = split_free(
        free_duration_s=free0_len * Params.BIN_SIZE,
        window_lengths_s=(1.0, 3.0, 5.0),
        gap_s=1.0,
        seed=None,
        start_offset_s=0.0,
    )

    # ----- split windows into 3 sequential blocks -----
    nW_total = len(windows0)
    nW_block = nW_total // 3
    if nW_block < 2:
        raise ValueError(f"Not enough windows in free0 to split into 3 blocks. nW_total={nW_total}")

    # trim so each block has equal count
    windows0 = windows0[: 3 * nW_block]

    windows_free0_0 = windows0[0            : nW_block]
    windows_free0_1 = windows0[nW_block     : 2 * nW_block]
    windows_free0_2 = windows0[2 * nW_block : 3 * nW_block]

    free1_len = get_n_time(prep_df, "free1", field=field)  # in bins
    windows1 = split_free(
        free_duration_s=free1_len * Params.BIN_SIZE,
        window_lengths_s=(1.0, 3.0, 5.0),
        gap_s=1.0,
        seed=None,
        start_offset_s=0.0,
    ...
    # trim so each block has equal count
    windows1 = windows1[: 3 * nW_block]

    windows_free1_0 = windows1[0            : nW_block]
    windows_free1_1 = windows1[nW_block     : 2 * nW_block]
    windows_free1_2 = windows1[2 * nW_block : 3 * nW_block]

    free_blocks = {
        "free0_0": {"trial_name": "free0", "windows": windows_free0_0},
        "free0_1": {"trial_name": "free0", "windows": windows_free0_1},
        "free0_2": {"trial_name": "free0", "windows": windows_free0_2},
        "free1_0": {"trial_name": "free1", "windows": windows_free1_0},
        "free1_1": {"trial_name": "free1", "windows": windows_free1_1},
        "free1_2": {"trial_name": "free1", "windows": windows_free1_2},

    }

    # ----- early intertrial block with SAME number of "windows" (trials) as each free block -----
    it = pyal.select_trials(prep_df, "trial_name=='intertrial'").reset_index(drop=True)

    if len(it) < nW_block:
        raise ValueError(f"Not enough intertrial trials ({len(it)}) for early block size nW_block={nW_block}")


    start = (len(it) - nW_block) // 2
    it_middle = it.iloc[start:start + nW_block].reset_index(drop=True)

    it_early = it.head(nW_block).reset_index(drop=True)
    it_late  = it.tail(nW_block).reset_index(drop=True)
    ...
    # ensure bhv exists inside this block too (safe even if already present)
    it_early, _ = dt.add_bhv(it_early, bhv_fields=["all_keypoints"])

    inter_blocks = {
        "inter_early": it_early,
        "inter_middle": it_middle,
        "inter_late": it_late

    }

    # ----- run decoding across the 4 blocks -----
    save_path = f"/home/il620/earthquake-analysis/notebooks/{sessions[session_idx]}_decode_pairs_cv_MOp_free0split3_interearly_ra.csv"
    ...
    save_path = f"/home/il620/earthquake-analysis/notebooks/{sessions[session_idx]}_decode_pairs_cv_VAL_free0split3_interearly_ra.csv"

    df_res, df_summary = run_cv_decode_pairs_incremental_free2(
        prep_df=prep_df,
        free_blocks=free_blocks,
        inter_blocks=inter_blocks,
        neural_field="VAL_rates",
        kin_col="bhv",
        n_splits=5,
        seed=0,
        n_components=30,
    ...
    # save_path = f"/home/il620/earthquake-analysis/notebooks/{sessions[session_idx]}_decode_pairs_cv_MOp_.csv"
