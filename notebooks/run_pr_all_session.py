...
sessions = [
    ...
    "M078_2025_08_06_15_30",
]

# ========================================== Linear ===========================================

results_path = "/home/il620/earthquake-analysis/notebooks"
step_size = 10
n_iter = 20

for session in sessions:
    prs_with_variable_neurons[session] = {}
    df = dt.load_sessions([session], prep = True, only_trials = False)[0]

    for area in areas:
        print(f"Processing area {area}")
        if f"{area}_rates" not in df.columns:
            continue

        prs_with_variable_neurons[session][area] = {}
        for condition in ["free0", "intertrial", "trial","free1"]:
            print(f"\tProcessing condition {condition}")
            tmp_df = pyal.select_trials(df, df.trial_name == condition)


            if condition == "intertrial" or condition == "free0" or condition == "free1":
                ...
                prs_with_variable_neurons[session][area][condition]["PR"] = (
                    part.get_pr_for_subsets_of_neurons(rates)
                )
            elif condition == "trial":
                prs_with_variable_neurons[session][area][condition] = {}

                tmp_df = pyal.restrict_to_interval(
                    tmp_df, epoch_fun=Params.perturb_epoch
                )
                rates = pyal.concat_trials(tmp_df, f"{area}_rates")
                prs_with_variable_neurons[session][area][condition]["neurons"] = np.arange(
                    5, rates.shape[1] + 1, step_size
        ...
        with open(results_path + "pr_all_sess_diff_num_neurons_perturb.pkl", "wb") as f:
