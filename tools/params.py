...
class Params:
    ...
    WINDOW_perturb_s = (0, 0.5)
    perturb_epoch_s = pyal.generate_epoch_fun(
        start_point_name="idx_sol_on",
        rel_start=int(WINDOW_perturb_s[0] / BIN_SIZE),
        rel_end=int(WINDOW_perturb_s[1] / BIN_SIZE),
    )
    ...
    WINDOW_perturb_l = (-0.2, 1.5)
    perturb_epoch_l = pyal.generate_epoch_fun(
        start_point_name="idx_sol_on",
        rel_start=int(WINDOW_perturb_l[0] / BIN_SIZE),
        rel_end=int(WINDOW_perturb_l[1] / BIN_SIZE),
    )
    WINDOW_perturb_ll = (-0.5, 2)
    perturb_epoch_ll = pyal.generate_epoch_fun(
        start_point_name="idx_sol_on",
        rel_start=int(WINDOW_perturb_ll[0] / BIN_SIZE),
        rel_end=int(WINDOW_perturb_ll[1] / BIN_SIZE),
    )
