...
def trainMultiRegionRNN(
    ...
    iWN = ampWN * npr.randn(number_units, len(tRNN))  # White noise matrix
    inputWN = np.ones((number_units, len(tRNN)))  # Initial OU variable
    for tt in range(1, len(tRNN)):
        inputWN[:, tt] = iWN[:, tt] + (inputWN[:, tt - 1] - iWN[:, tt]) * np.exp(
            -(dtRNN / tauWN)
    ...
    J = (
        g * npr.randn(number_units, number_units) / math.sqrt(number_units)
    ...
    Adata = np.maximum(Adata, -0.999)  # Shape: N x tData
    stdData = np.std(Adata[iTarget, :])  # get standard deviation of entire data
